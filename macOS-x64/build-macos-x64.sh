...
APPLICATION_PATH=${TARGET_DIRECTORY}/application
...
function codeSignFiles() {
  log_info "Application installer codesign files started.(1/5)"

  read -p "Please enter the Developer Application ID: " DEVELOPER_ID_APPLICATION

  ${TARGET_DIRECTOR}/darwin/scripts/codesign_files $APPLICATION_PATH $DEVELOPER_ID_APPLICATION
}

function buildPackage() {
  log_info "Apllication installer package building started.(2/5)"
...
function buildProduct() {
  log_info "Application installer product building started.(3/5)"
...
function signProduct() {
  log_info "Application installer signing process started.(4/5)"
  mkdir -p ${TARGET_DIRECTORY}/pkg-signed
  chmod -R 755 ${TARGET_DIRECTORY}/pkg-signed

  read -p "Please enter the Apple Developer ID Installer: " DEVELOPER_ID_INSTALLER

  productsign --sign "Developer ID Installer: ${DEVELOPER_ID_INSTALLER}" \
...
function notarizePackage() {
  log_info "Application installer notarization started.(5/5)"

  read -p "Please enter the Apple Developer E-mail Address: " DEVELOPER_EMAIL_ADDRESS

  xcrun altool \
    --notarize-app \
    --primary-bundle-id "org.haskell.ghc" \
    --username "${DEVELOPER_EMAIL_ADDRESS}" \
    --password "@keychain:AC_PASSWORD" \
    --file ${TARGET_DIRECTORY}/pkg-signed/$1
}

function createInstaller() {
  log_info "Application installer generation process started.(5 Steps)"

  codeSignFiles
  buildPackage
  buildProduct ${PRODUCT}-${VERSION}-x86_64-apple-darwin.pkg
  ...
  [[ $FLAG == "true" ]] && signProduct ${PRODUCT}-${VERSION}-x86_64-apple-darwin.pkg
